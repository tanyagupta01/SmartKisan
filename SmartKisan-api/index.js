...
app.post('/api/ask', async (req, res) => {
  ...
  try {
    ...
    // 2) TTS generation (AUDIO only) - Fixed to use proper model and structure
    const ttsResp = await ai.models.generateContent({
      model: 'gemini-2.5-flash-preview-tts', // Use the correct TTS model
      contents: [{ parts: [{ text: replyText }] }],
      config: {
        responseModalities: ['AUDIO'],
        speechConfig: {
    ...
    // 3) Return both text and audio - Fixed variable name
    res.json({
      reply: replyText,
      audioContent: audioBase64 // Fixed: was using undefined variable
    });
